...
public class PmcParser {
    ...
    /**
     * Test specific functions of dataWriter. Just for development stages.
     * @param args 
     */
    public static void main(String[] args) {
        String ipath = "D:\\42 IASIS tmp files\\AD_20200122\\pmc\\XMLs";
        MongoDatasetConnector mdcAD = new MongoDatasetConnector("localhost",27017,"iasis","LC_PMC_20171011");

        PmcParser pc = new PmcParser(ipath);
        //              Write all in one JSON file may cause "out of memory exception"
        pc.xmlsToMongo(ipath, mdcAD);
    }
    /**
    ...
    public void xmlsToJsons(String basePath, String targetPath) {
        Path path = Paths.get(basePath);
        System.out.println("basePath="+path.toString());
            ...
            try {
                Files.walkFileTree(path, new SimpleFileVisitor<Path>() {
                    ...
                    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                        ...
                        //Write JSON file
                        Helper.writeJsonFile(targetPath + pathDelimiter + file.getFileName().toString().replace(".xml", ".json"),result );
                        return FileVisitResult.CONTINUE;
                    }
                });
            } catch (IOException ex) {
